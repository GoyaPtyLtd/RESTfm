...
class Request {
     ...
     * </dl>
     *
     *     acceptFormats An array of possible accepted format extensions allowed at end of URI. - GOYA
     *
    ...
    function __construct($config = array()) {
        ...
        $config['acceptFormats'] = $this->getConfig($config, 'acceptFormats', NULL, array());
        ...
        // GOYA - This implementation is broken for URIs with multiple dots '.'
        //        in the last 'part'. The intent appears to be for the handling
        //        of language codes.
        //$parts = explode('.', $lastPart);
        //$this->uri .= '/'.$parts[0];

        // GOYA - Instead only strip the format extensions that match the
        //        acceptFormats list. Set up the variables to otherwise work
        //        as per previous implementation.
        $matches = array();
        preg_match('/(.*?)(\.([^\.]*))?$/', $lastPart, $matches);
        $extension = isset($matches[3]) ? $matches[3] : NULL;
        $parts = array($matches[1]);
        if (in_array($extension, $config['acceptFormats'])) {
            // OK to exclude this accepted format extension from end of URI.
            $this->uri .= '/'.$matches[1];
            $parts[] = $extension;
        } else {
            // Keep URI completely intact.
            $this->uri .= '/'.$lastPart;
        }
        // GOYA
        ...
        /*
        // GOYA - temporary data debugging.
        $tmpfname = tempnam(sys_get_temp_dir(), 'RESTfm.debug.');
...
class Resource {
    ...
    function exec($request) {
        ...
        // GOYA - Don't need this in production
